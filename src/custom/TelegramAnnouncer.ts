...
export class TelegramAnnouncer {
    ...
    private draftMessages: string[] = ['Alright folks, let\'s draft: PLAYER_NAMES'];
    private researchMessages: string[] = ['Пора инвестировать в говно, парни: PLAYER_NAMES'];
    ...
    public sendDraft(game: IGame) {
        if (this.sendDraftFlag) {
            const msg = this.getRandomMessage(
                game.players.map(p => this.getTelegramName(p.name)).join(', '),
                'PLAYER_NAMES',
                this.draftMessages
            );
    ...
    public sendNewPlayerTurn(playerName: string) {
        if (this.sendNewTurnFlag) {
            const msg = this.getRandomMessage(
                this.getTelegramName(playerName),
                'PLAYER_NAME',
    ...
    private getRandomMessage(tags: string, toReplace: string, messages: readonly string[]): string {
        const randomIndex = Math.floor(Math.random() * messages.length);
        return messages[randomIndex].replaceAll(toReplace, tags);
    }
    ...
    private parseEnvData() {
        ...
        this.turnAnnouncements = process.env.TELEGRAM_ANNOUNCE_TURN_MESSAGES
            ? JSON.parse(process.env.TELEGRAM_ANNOUNCE_TURN_MESSAGES)
            : this.turnAnnouncements;

        this.draftMessages = process.env.TELEGRAM_DRAFT_MESSAGES
            ? JSON.parse(process.env.TELEGRAM_DRAFT_MESSAGES)
            : this.draftMessages;

        this.researchMessages = process.env.TELEGRAM_RESEARCH_MESSAGES
            ? JSON.parse(process.env.TELEGRAM_RESEARCH_MESSAGES)
            : this.researchMessages;
